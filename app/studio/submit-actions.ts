...
export async function submitSongJob(raw: unknown): Promise<SubmitResult | void> {
  ...
  /**
   * The bot challenge that used to run here (Turnstile) came off 2026-09-23, Henry's call:
   * this action only ever runs for a signed-in customer, and the sign-in form ahead of it
   * keeps its own challenge, so the gate this relies on now is the session plus the existing
   * rate limits, not a second widget on an already-authenticated path.
   */
