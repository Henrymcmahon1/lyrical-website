 ...
 *
 * Every action re-checks `requireAdmin()`, the same discipline the rest of `/admin` uses:
...
export async function addPersonNote(formData: FormData) {
  ...
  await addNote({ userId, body, author: adminEmail(admin) })
  revalidatePath('/admin')
}
...
export async function addPersonTask(formData: FormData) {
  ...
  await addTask({ userId, title, dueOn: dueOn || undefined, owner: adminEmail(admin) })
  revalidatePath('/admin')
}
