...
const { FeedbackTab, FEEDBACK_COLUMNS, flattenFeedback } = await import('@/app/admin/FeedbackTab')
const ROW = {
...
describe('Feedback tab', () => {
  it('draws the job, the rating, the tags, the note and a link to the job', async () => {
    ...
    for (const s of ['A Song', 'An Artist', 'EN to ES', 'Thumbs down', 'timing', 'Chorus line 2 lands late', 'href="/admin?tab=work&amp;show=all#j1"']) expect(h).toContain(s)
  })
