...
const { saveEmailPreferencesForm, deleteAccountForm, updateNameForm } = await import('@/app/studio/account/actions')
...
describe('updateNameForm', () => {
  it('refuses without a session, and touches nothing', async () => {
    currentUser.mockResolvedValueOnce(null)
    const fd = new FormData()
    fd.set('name', 'Jamie Rivers')
    const r = await updateNameForm(null, fd)
    expect(r.ok).toBe(false)
    expect(updateName).not.toHaveBeenCalled()
  })

  it('saves the caller\'s name and revalidates on success', async () => {
    updateName.mockResolvedValue({ ok: true, name: 'Jamie Rivers' })
    const fd = new FormData()
    fd.set('name', 'Jamie Rivers')
    const r = await updateNameForm(null, fd)
    expect(updateName).toHaveBeenCalledWith({ marker: 'admin' }, 'user-1', 'Jamie Rivers')
    expect(r).toEqual({ ok: true, name: 'Jamie Rivers' })
    expect(revalidatePath).toHaveBeenCalledWith('/studio/account')
  })

  it('reports a refused save without revalidating', async () => {
    updateName.mockResolvedValue({ ok: false, error: 'Enter a name.' })
    const r = await updateNameForm(null, new FormData())
    expect(r).toEqual({ ok: false, error: 'Enter a name.' })
    expect(revalidatePath).not.toHaveBeenCalled()
  })
})
